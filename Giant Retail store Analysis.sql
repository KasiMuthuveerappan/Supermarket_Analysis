...
select * from `getrat`.INFORMATION_SCHEMA.TABLES;

select column_name,data_type
from `getrat`.INFORMATION_SCHEMA.COLUMNS
...
from `getrat.customers` c
join `getrat.orders` o 
...
from `getrat.customers` c
join `getrat.orders` o on c.customer_id=o.customer_id
join`getrat.order_items` oi on oi.order_id = o.order_id
group by customer_state
order by customer_state;
...
from `getrat.customers` c
join `getrat.orders` o on c.customer_id=o.customer_id
join `getrat.order_items` as p on o.order_id = p.order_id
group by customer_state
order by 3 desc
limit 5 
)
...
from `getrat.customers` as c
join `getrat.orders` as o on c.customer_id = o.customer_id
join `getrat.order_items` as p on o.order_id = p.order_id
group by customer_state
) nt1
...
from `getrat.order_items` as o
join `getrat.orders` as od on o.order_id = od.order_id
join `getrat.customers` as c on od.customer_id = c.customer_id
...
FROM `getrat.orders` AS o
join `getrat.payments` AS p ON o.order_id = p.order_id
where order_status not in ('canceled','unavailable')
GROUP BY Month
ORDER BY Month;
...
from `getrat.products`p
join `getrat.order_items`o
...
from `getrat.order_reviews`rw
join `getrat.order_items`oi on oi.order_id=rw.order_id
join `getrat.products`p on p.product_id=oi.product_id
group by 1
